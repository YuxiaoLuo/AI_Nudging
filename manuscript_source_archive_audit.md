...
- Current issue:
  - no authoritative local PDF archived yet
- Current crosswalk note:
  - Exact citation is confirmed in `literature_reading_matrix.md` and `literature/download_log.md`, but no authoritative local PDF is currently present. The direct Taylor & Francis PDF route and ResearchGate route returned `403` access blocks on 2026-06-22, and a 2026-06-23 browser-header retry against the Taylor & Francis PDF endpoint still saved an HTML document beginning with `<!DOCTYPE html>` rather than a real PDF. A later metadata recheck via OpenAlex also exposed no open-access PDF route for this DOI. A 2026-07-01 repository recheck further clarified that the ResearchGate page currently shows `Request full-text PDF` and `No full-text available`, so no public repository-hosted full text is exposed there either. A 2026-07-02 open-web recheck sharpened the diagnosis slightly further: current Taylor & Francis search snippets explicitly advertise `View PDF` / `Download PDF`, but no usable public PDF route emerged from this environment. A later 2026-07-02 canonical-endpoint recheck confirmed that both the Taylor & Francis PDF and abstract routes still resolve to challenge HTML from this environment, so the gap now looks even more clearly like access-gated retrieval rather than route discovery. The download log records those routes for later manual retrieval, and `literature/DSS2024_XAI & Decision Making.pdf` should not be treated as a verified substitute.
