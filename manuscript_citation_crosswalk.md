...
| Ebrahimi et al. (2022) | Supports the claim that interactivity is itself a meaningful recommendation-system design dimension rather than a superficial interface flourish. | Bridge/support | Theoretical Background | Exact citation is confirmed in `literature_reading_matrix.md` and `literature/download_log.md`, but no authoritative local PDF is currently present. The reachable JMIS page did not expose a directly downloadable PDF from this environment on 2026-06-22; the download log now records Taylor & Francis and ResearchGate routes for later manual retrieval, and `literature/DSS2024_XAI & Decision Making.pdf` should not be treated as a verified substitute. |
...
- The latest archive attempt on `2026-06-22` suggests the remaining gap is publisher-access or route availability from this environment, not bibliographic uncertainty.
